...
//        }
    ...
    public function cash_transaction_export(Request $request)
    {
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestaurantWiseCashCollectionExport($data), 'Transação em dinheiro.csv');
            }
            return Excel::download(new RestaurantWiseCashCollectionExport($data), 'Transação em dinheiro.xlsx');
        } catch(\Exception $e) {
    ...
    public function digital_transaction_export(Request $request)
    {
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestaurantWiseWithdrawOrderTransactionExport($data), 'Transação de pedido admin.csv');
            }
            return Excel::download(new RestaurantWiseWithdrawOrderTransactionExport($data), 'Transação de pedido admin.xlsx');
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
    ...
    public function withdraw_transaction_export(Request $request)
    {
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestaurantWiseWithdrawTransactionExport($data), 'Transação Retirada.csv');
            }
            return Excel::download(new RestaurantWiseWithdrawTransactionExport($data), 'Transação Retirada.xlsx');
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
    ...
    public function restaurants_export( Request $request,  $type)
    {
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestaurantListExport($data), 'Açaiterias.csv');
            }
            return Excel::download(new RestaurantListExport($data), 'Açaiterias.xlsx');
    ...
    public function restaurant_wise_reviwe_export(Request $request){
        try{
            ...
            if($request->type == 'csv'){
                return Excel::download(new RestauranrWiseFoodReviewExport($data), 'Ponto de Venda por avaliação de comidas.csv');
            }
            return Excel::download(new RestauranrWiseFoodReviewExport($data), 'Pontos de Venda por avaliação de comidas.xlsx');
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
            return back();
    ...
    public function withdraw_list_export(Request $request)
    {
        ...
        $withdraw_req = WithdrawRequest::with(['vendor'])
            ...
            if ($request->type == 'excel') {
                return Excel::download(new RestaurantWithdrawTransactionExport($data), 'Solicitações retirada.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new RestaurantWithdrawTransactionExport($data), 'Solicitações retirada.csv');
            }
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
    ...
    public function disbursement_export(Request $request,$id,$type)
    {
        ...
        if ($request->type == 'excel') {
            return Excel::download(new DisbursementHistoryExport($data), 'Lista de pagamentos.xlsx');
        } else if ($request->type == 'csv') {
            return Excel::download(new DisbursementHistoryExport($data), 'Lista de pagamentos.csv');
        }
    }
