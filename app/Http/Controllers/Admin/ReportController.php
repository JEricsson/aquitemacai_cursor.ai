    ...
    public function day_wise_report_export(Request $request){
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new TransactionReportExport($data), 'Relatório de transação.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new TransactionReportExport($data), 'Relatório de transação.csv');
            }
        }  catch(\Exception $e)
        {
    ...
    public function food_wise_report_export(Request $request){
        try{
            ...
            if ($request->export_type == 'excel') {
                return Excel::download(new FoodReportExport($data), 'Relatório de comida.xlsx');
            } else if ($request->export_type == 'csv') {
                return Excel::download(new FoodReportExport($data), 'Relatório de comida.csv');
            }
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
...
//        dd($expense);
    ...
    public function expense_export(Request $request)
    {
        try {
            ...
            if ($request->type == 'excel') {
                return Excel::download(new ExpenseReportExport($data), 'Relatório de despesas.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new ExpenseReportExport($data), 'Relatório de despesas.csv');
            }
    ...
    public function subscription_export(Request $request)
    {
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new SubscriptionReportExport($data), 'Exportação de relatório de assinatura.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new SubscriptionReportExport($data), 'Exportação de relatório de assinatura.csv');
            }
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
    ...
    public function campaign_report_export(Request $request)
    {
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new CampaignReportExport($data), 'Relatório de pedidos de campanha.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new CampaignReportExport($data), 'Relatório de pedidos de campanha.csv');
            }
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
    ...
    public function order_report_export(Request $request)
    {
        try{
            ...
            if ($request->type == 'excel') {
                return Excel::download(new OrderReportExport($data), 'Relatório do pedido.xlsx');
            } else if ($request->type == 'csv') {
                return Excel::download(new OrderReportExport($data), 'Relatório do pedido.csv');
            }
        } catch(\Exception $e) {
            Toastr::error("line___{$e->getLine()}",$e->getMessage());
            info(["line___{$e->getLine()}",$e->getMessage()]);
    ...
    public function restaurant_export(Request $request)
    {
        ...
        if ($request->export_type == 'csv') {
            return Excel::download(new RestaurantSummaryReportExport($data), 'Relatório do Açaiteria.csv');
        }
        return Excel::download(new RestaurantSummaryReportExport($data), 'Relatório do Açaiteria.xlsx');
    ...
    public function disbursement_report_export(Request $request,$type,$tab = 'restaurant')
    {
        ...
        if($type == 'csv'){
            return Excel::download(new DisbursementReportExport($data), 'Relatório de pagamento.csv');
        }
        return Excel::download(new DisbursementReportExport($data), 'Relatório de pagamento.xlsx');
